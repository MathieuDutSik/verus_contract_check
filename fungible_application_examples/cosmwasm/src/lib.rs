// CosmWasm fungible-token contract with Verus-verified core arithmetic
// and a cw20-style allowance / mint / burn surface.
//
// All verified helpers — transfer, instantiate, balance_of, allowance,
// approve, transfer_from, mint, burn, increase_allowance,
// decrease_allowance, update_minter — live in `verified_helpers.rs`.
// The SDK-decorated `instantiate` / `execute` / `query` entry points
// below are thin forwarders that wrap CosmWasm's `&mut dyn Storage` in
// a `StoreRef` adapter and map the verified `TransferError` onto a
// chain-specific `ContractError`.
//
// Layout (mirrors `linera_alternate` fungible):
//   - `pub mod core;`              — chain-agnostic verified core.
//   - `pub mod cw_axioms;`         — CosmWasm runtime axioms.
//   - `pub mod verified_helpers;`  — all verified_* functions plus
//                                    `allowance_at` spec helper and
//                                    re-export of `TransferError`.
...
use crate::cw_axioms::TOTAL_SUPPLY;
use crate::verified_helpers::{
    verified_allowance, verified_approve, verified_balance_of, verified_burn,
    verified_decrease_allowance, verified_increase_allowance, verified_instantiate,
    verified_mint, verified_transfer, verified_transfer_from, verified_update_minter,
};
